...
public class GeografijaDAO {
    ...
    private final PreparedStatement dajGradove;
    private final PreparedStatement dajDrzave;
    private final PreparedStatement ispisGradova;
    ...
    private static final ObjectProperty trenutniGrad=new SimpleObjectProperty();
    private GeografijaDAO() throws SQLException , ClassNotFoundException{
        ...
        dajGradove=conn.prepareStatement("SELECT g.* , d.naziv Dnaziv FROM Grad g, Drzava d WHERE g.drzava=d.id and d.glavni_grad=g.id ORDER BY g.broj_stanovnika DESC");
    ...
    while(rez.next()){
        gradovi.add(new Grad(rez.getInt(1),rez.getString(2),rez.getInt(3),rez.getInt(4)));
    }
        while(rez.next()){
            drzave.add(new Drzava(rez.getInt(1),rez.getString(2),rez.getInt(3)));
        }
   ...
   public  Drzava nadjiDrzavu(String drzava) throws SQLException{
    ...
    public  ObservableList<Grad> getGradovi() throws SQLException{
        return gradovi;
    }
    public void setGradovi(ObservableList<Grad> g) {
        gradovi = g;
        try {
            PreparedStatement p = conn.prepareStatement("DELETE FROM Grad");
            p.executeUpdate();
            p=conn.prepareStatement("INSERT INTO Grad VALUES (? ? ?)");
            for(int i=0;i<g.size();i++){
                //p.setInt(1,g.get(i).getId());
                p.setString(1,g.get(i).getNaziv());
                p.setInt(2,g.get(i).getBrojStanovnika());
                p.setInt(3,g.get(i).getDrzava());
                p.executeUpdate();
            }
        }
        catch(SQLException e){
    ...
    public  ObservableList<Drzava> getDrzave() throws SQLException{
        return drzave;
    }
    public void setDrzave(ObservableList<Drzava> d){
       drzave=d;
    }
public Connection getConnection(){return conn;}

public static Grad getTrenutniGrad(){return (Grad)trenutniGrad.get();}

    public void setTrenutniGrad(Grad g){trenutniGrad.set(g);}




public synchronized void izbrisiDrzave() throws SQLException{
        PreparedStatement ps = conn.prepareStatement("DELETE Drzava");
        ps.executeUpdate();
}

public synchronized void izbrisiGradove() throws SQLException{
        PreparedStatement ps = conn.prepareStatement("DELETE Grad");
        ps.executeUpdate();
}
